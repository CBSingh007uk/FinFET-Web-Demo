...
def synthetic_parameters():
    ...
    data = [
        ...
        {"Node":"4nm","Lg (nm)":9,"Hfin (nm)":50,"EOT (nm)":0.50,"ID (A/cm²)":2.3e4,
         "Vth":0.28,"Ion/Ioff":4.0e6,"gm":3100,"Rsd":60,"Cgg":1.4,"Delay":0.8},
        {"Node":"3nm","Lg (nm)":7,"Hfin (nm)":55,"EOT (nm)":0.48,"ID (A/cm²)":2.6e4,
         "Vth":0.25,"Ion/Ioff":5.0e6,"gm":3400,"Rsd":50,"Cgg":1.6,"Delay":0.6},
        {"Node":"2nm","Lg (nm)":5,"Hfin (nm)":60,"EOT (nm)":0.45,"ID (A/cm²)":3.0e4,
         "Vth":0.22,"Ion/Ioff":6.0e6,"gm":3600,"Rsd":40,"Cgg":1.8,"Delay":0.5},
    ]
...
if option == "Synthetic Demo":
    ...
    if tables:
        for page_num, df in tables:
            st.subheader(f"Table from Page {page_num}")
            st.dataframe(df)
    else:
        st.warning("No tables detected. PDF quality may affect extraction.")
